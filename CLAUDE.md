# Claude Code Project Instructions — predicates-kotlin

## Project Overview

This is **predicates-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`predicates`](https://crates.io/crates/predicates).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/predicates/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/predicates>
