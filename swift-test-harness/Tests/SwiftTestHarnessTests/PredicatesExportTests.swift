...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Predicates.swiftmodule/`
...
//      `import Predicates` above would fail at compile time.
//
//   2. The static archive `libPredicates.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Predicates" }` and
//      `flattenPackage = "io.github.kotlinmania.predicates"` configuration in
...
//      via the `PredicatesLibrary` product.
//
...
final class PredicatesExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Predicates swift module imported cleanly")
    }
}
